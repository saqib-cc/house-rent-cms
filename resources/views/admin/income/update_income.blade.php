         ...
         <div class="row">  
           <div class="col-md-8 offset-2 mt-5">
            <div class="card card-primary">
                ...
                <!-- /.card-header -->
                <!-- form start -->
                <form method="post" action="{{url('update_income',$income->id)}}" enctype="multipart/form-data">
                 @csrf
                  <div class="card-body">
                      ...
                      <select name="category" id="category" class="form-control">
                        @if(isset($categories))
                          @foreach ($categories as $item)
                              <option @if($income->category_id==$item->id) {{ 'selected'}} @endif value="{{$item->id}}">{{$item->name}}</option>
                          @endforeach
                        @endif  
                      </select>
                      @error('category')
                    ...
                    <div class="form-group">
                      ...
                      <select name="subcategory" id="sub_category" class="form-control" sub_category='{{$income->subcategory->id}}'>
                    ...
                    <div class="form-group">
                      ...
                      <input type="date" class="form-control" name="date"  value="{{$income->date}}">
                      @error('date')
                        <span class="text-danger">{{$message}}</span>    
                      @enderror
                    </div>
